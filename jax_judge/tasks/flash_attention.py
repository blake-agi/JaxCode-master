...
never materializing the full $seq_q \times seq_k$ score matrix, and produce output
...
- Handle a `seq_k` that is not a multiple of `block_size`
...
$O(seq^2)$ score matrix out to HBM and reads it back for the softmax and again for
the $V$ multiply. Flash keeps each tile in SRAM and never writes the scores at
all, taking HBM traffic from $\Theta(seq^2 + Td)$ words down to
$\Theta(seq^2 d^2 / M)$, where $M$ is the SRAM capacity in words (Dao et al.,
2022, Thm. 2). Since $M \gg d^2$ on real accelerators, that is a large
constant-factor cut — it is still quadratic in $seq$.

The memory consequence is the bigger deal in practice: peak activation memory
for attention drops from $O(seq^2)$ to $O(seq)$, which is what made long context
...
def flash_attention(Q, K, V, block_size=16):
    ...
    Returns:
        (seq_q, d_v) — the same function as standard attention, for any block_size.
    """
    pass  # Replace this
''',
    "solution": '''import jax
...
def flash_attention(Q, K, V, block_size=16):
    ...
    m = jnp.full((seq_q, 1), -jnp.inf, dtype=Q.dtype)
    ell = jnp.zeros((seq_q, 1), dtype=Q.dtype)
    acc = jnp.zeros((seq_q, d_v), dtype=Q.dtype)

    for start in range(0, seq_k, block_size):
        stop = min(start + block_size, seq_k)          # handles a ragged last tile
        k_blk = K[start:stop]
        v_blk = V[start:stop]

        s = (Q @ k_blk.T) * scale                    # (seq_q, blk)
